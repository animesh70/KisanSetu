...
import { ArrowRight, Bell, Boxes, CircleDollarSign, Leaf, MapPin, Menu, PackagePlus, ShieldCheck, Sparkles, TrendingUp, Truck, Users, CheckCircle2, Clock3, MessageSquareWarning, Trash2, Wrench, Store } from 'lucide-react';
...
import DirectMarketplace from './components/DirectMarketplace.jsx';
import TransactionRouteModal from './components/TransactionRouteModal.jsx';
...
const NAV_SECTIONS = [
  ...
  { label: 'Direct marketplace', id: 'direct-marketplace' },
...
export default function App() {
  ...
  const [routeTransactionId, setRouteTransactionId] = useState('');
  ...
  const moveTransaction = async (transaction) => {
    if (transaction.escrowStatus && transaction.status === 'delivered' && transaction.escrowStatus !== 'released') {
      showNotice('alerts.escrowOtpRequired');
      return;
    }
  ...
  return <div className="app-shell">
    ...
    <main id="dashboard"><div className="main-content-inner"><header id="dashboard-overview" className="topbar"><button className="mobile-menu" aria-label={t('nav.dashboard')} aria-expanded={mobileNavOpen} onClick={() => setMobileNavOpen(true)}><Menu/></button><div><p className="eyebrow">{t('greeting')} <span className="demo-chip">{t('page.demoData')}</span></p><h1>{t('headline')}</h1></div><div className="top-actions"><select className="language-select" value={i18n.language} onChange={(event) => i18n.changeLanguage(event.target.value)} aria-label={t('page.language')}>{LANGUAGE_OPTIONS.map((language) => <option key={language.code} value={language.code}>{language.label}</option>)}</select><button className="farmer-mode-button" onClick={() => setFarmerMode(!farmerMode)}>{farmerMode ? t('page.standardText') : t('page.farmerMode')}</button><div className="notification-wrap"><button className="notification" aria-label={`${notifications.length} ${t('page.notifications')}`} aria-expanded={showNotifications} onClick={() => setShowNotifications(!showNotifications)}><Bell size={20}/>{notifications.length > 0 && <i/>}</button>{showNotifications && <div className="notification-panel" role="status"><div className="notification-panel-head"><div><strong>{t('page.notifications')}</strong><small>{t('page.recentUpdates', { count: notifications.length })}</small></div><button type="button" aria-label={t('page.closeNotifications')} onClick={() => setShowNotifications(false)}>×</button></div><div className="notification-list">{notifications.map((item) => <div className="notification-item" key={item.id}><span className="notification-dot"/><div><strong>{t(item.titleKey, item.params)}</strong><p>{t(item.detailKey, item.params)}</p></div></div>)}</div><button className="notification-review" type="button" onClick={() => { setShowNotifications(false); navigateTo('Transactions'); }}>{t('page.reviewOffers')} <ArrowRight size={14}/></button></div>}</div><button className="help-button" onClick={resetDemo}>{t('reset')}</button></div></header>
        ...
        return <article className="lot-card" key={lot.id}>
          ...
          {shared && <div className={`shared-logistics-panel ${sharedStatus === 'available' ? 'is-saving' : ''}`} role="status">
            ...
            {!shared.error && shared.eligible && <>
              ...
              {shared.group && shared.nearbyLotCount > 0 && <>
                <p>{t('sharedLogistics.totalQuantity')}: <strong>{shared.group.totalQuantity} q</strong></p>
                {shared.group.shareRecommended && <>
                  <p>{t('sharedLogistics.recommendedVehicle')}: <strong>{shared.group.transportProvider} · {shared.group.vehicleCapacity} q</strong> · {t('sharedLogistics.trips', { count: shared.group.sharedTrips })}</p>
                  <div className="shared-logistics-metrics">
                    <span><small>{t('sharedLogistics.soloCost')}</small><strong>{formatPrice(shared.group.estimatedSoloCost)}</strong></span>
                    <span><small>{t('sharedLogistics.sharedCost')}</small><strong>{formatPrice(shared.group.estimatedSharedCost)}</strong></span>
      ...
      <DirectMarketplace onChanged={refreshWorkflow} refreshToken={transactions.map((item) => `${item.id}:${item.status}:${item.escrowStatus || ''}`).join('|')}/>
      ...
      {offers.filter((offer) => offer.status === 'pending').map((offer) => {
        ...
        const remaining = lot ? Number(lot.quantity) - Number(offer.quantity) : null;
        return <section className="transaction-card" key={offer.id}><div className="transaction-status">{t('page.demoBuyerOffer')}</div><div><strong>{lot?.crop || t('page.cropLot')} · {lot?.quantity ?? '—'} q {t('offerDetails.lot')}</strong><p>{t('page.buyerOffer')}: {offer.quantity} q {t('offerDetails.at')} {formatPrice(offer.pricePerUnit)} {t('page.perQuintal')} · {remaining === 0 ? t('offerDetails.fullLot') : remaining > 0 ? t('offerDetails.remaining', { quantity: remaining }) : t('page.pickupAvailable')}</p>{counteringOffer === offer.id && <div className="counter-row"><input aria-label={t('page.counter')} type="number" min="1" placeholder="₹/q" value={counterPrice} onChange={(event) => setCounterPrice(event.target.value)}/><button className="outline-button" onClick={() => respondToOffer(offer.id, 'countered')}>{t('page.sendCounter')}</button></div>}</div><div className="offer-actions"><button className="outline-button" onClick={() => setCounteringOffer(counteringOffer === offer.id ? '' : offer.id)}>{t('page.counter')}</button><button className="outline-button" onClick={() => respondToOffer(offer.id, 'rejected')}>{t('page.decline')}</button><button className="primary-button" onClick={() => acceptOffer(offer.id)}>{t('page.acceptOffer')}</button></div></section>;
      })}
      ...
      {transactions.map((transaction) => <section className="transaction-card" key={transaction.id}><div className="transaction-status"><CheckCircle2 size={13}/> {t('page.payment')}</div><div><strong>{transaction.crop || t('page.cropLot')} · {formatPrice(transaction.amount)}</strong><p>{transaction.buyerName || t('page.verifiedBuyer')} · {transaction.quantity || '—'} {t('page.quintals')}</p><small className="payment-detail">{transaction.pickupWindow || transaction.logisticsProvider || t('page.logisticsOptions')} · {transaction.driverName || t('page.pickup')}</small><small className="payment-detail">{transaction.paymentMethod || t('page.payment')} · Ref: {transaction.paymentReference || '—'} · {transaction.paymentDue || '—'}</small>{transaction.escrowStatus && <small className="payment-detail escrow-transaction-detail"><ShieldCheck size={13}/> {t('marketplace.escrowActivity')}: {transaction.escrowStatus === 'funds_locked' ? t('marketplace.statusLocked') : transaction.escrowStatus === 'released' ? t('marketplace.statusReleased') : t('marketplace.statusAwaiting')} · {t('page.platformFee')} {transaction.platformFeePercent || 1.5}% ({formatPrice(transaction.platformFee)}) · {t('page.farmerPayout')} {formatPrice(transaction.farmerPayout ?? transaction.netPayable)}</small>}<div className="transaction-steps"><span className={['pickup_scheduled', 'in_transit', 'delivered', 'completed'].includes(transaction.status) ? 'done' : ''}>1. {t('page.pickup')}</span><span className={['in_transit', 'delivered', 'completed'].includes(transaction.status) ? 'done' : ''}>2. {t('page.transit')}</span><span className={['delivered', 'completed'].includes(transaction.status) ? 'done' : ''}>3. {t('page.delivery')}</span><span className={transaction.paymentStatus === 'paid' ? 'done' : ''}>4. {t('page.payment')}</span></div></div><div className="offer-actions">{transaction.status !== 'completed' && !(transaction.escrowStatus && transaction.status === 'delivered' && transaction.escrowStatus !== 'released') && <button className="primary-button" onClick={() => moveTransaction(transaction)}><Clock3 size={16}/> {t('page.nextStep')}</button>}{!transaction.escrowStatus && ['delivered', 'completed'].includes(transaction.status) && transaction.paymentStatus !== 'paid' && <button className="outline-button" onClick={() => confirmPayment(transaction)}>{t('page.confirmPayment')}</button>}<button className="outline-button transaction-route-button" onClick={() => setRouteTransactionId(transaction.id)}><MapPin size={16}/>{t('routePlanner.open')}</button><button className="quiet-button" onClick={() => downloadReceipt(transaction)}>{t('page.downloadReceipt')}</button></div></section>)}
    ...
    {routeTransactionId && transactions.find((item) => item.id === routeTransactionId) && <TransactionRouteModal transaction={transactions.find((item) => item.id === routeTransactionId)} onClose={() => setRouteTransactionId('')}/>}
    <KisanAssistant
