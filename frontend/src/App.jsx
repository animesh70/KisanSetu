...
import { ArrowRight, Bell, Boxes, CircleDollarSign, Leaf, MapPin, Menu, PackagePlus, ShieldCheck, Sparkles, TrendingUp, Truck, Users, CheckCircle2, Clock3, MessageSquareWarning, Landmark, BarChart3 } from 'lucide-react';
...
export default function App() {
  ...
  const loadDashboard = async () => {
    ...
    try {
      const [prices, trend, forecast, recommendation, buyers, online] = await Promise.all([
        api.getPrices(filters.crop, filters.location), api.getTrend(filters.crop), api.getForecast(filters.crop), api.getRecommendation(filters), api.getBuyers(), api.getOnlinePrices(filters.crop)
      ]);
      setData({ prices: prices.length ? prices : fallback.prices, trend, forecast, recommendation, buyers }); setOnlinePrices(online || []);
    } catch { setData(fallback); setOnlinePrices([]); setNotice('Showing demo data — start the KisanSetu backend to use live API results.'); }
  ...
  const navigateTo = (item) => {
    ...
    if (item === 'Smart Loans') { setShowLoanForm(true); return; }
    const targetId = { Dashboard: 'dashboard', 'Market prices': 'market-prices', 'Price comparison': 'price-comparison', 'Smart Loans': 'smart-loans', 'Buyer matches': 'buyer-matches', Logistics: 'logistics', Transactions: 'transactions' }[item];
    document.getElementById(targetId)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };

  const bestMarket = [...data.prices].sort((a, b) => b.modalPrice - a.modalPrice)[0];
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><span className="brand-mark"><Leaf size={23}/></span><div><strong>KisanSetu</strong><small>Market intelligence</small></div></div><nav>{[{ label: 'Dashboard', icon: Boxes }, { label: 'Market prices', icon: TrendingUp }, { label: 'Price comparison', icon: BarChart3 }, { label: 'Smart Loans', icon: Landmark }, { label: 'My crop lots', icon: PackagePlus }, { label: 'Buyer matches', icon: Users }, { label: 'Logistics', icon: Truck }, { label: 'Transactions', icon: CircleDollarSign }].map(({ label, icon: Icon }) => <button type="button" key={label} className={activeNav === label ? 'active' : ''} onClick={() => navigateTo(label)}><Icon size={19}/>{label}</button>)}</nav><div className="sidebar-footer"><div className="profile-avatar">SP</div><div><strong>Sanjay Patil</strong><small>Farmer · Nashik</small></div></div></aside>
    <main id="dashboard"><header className="topbar"><button className="mobile-menu" onClick={() => navigateTo('Dashboard')}><Menu/></button><div><p className="eyebrow">GOOD MORNING, SANJAY</p><h1>Make every harvest count.</h1></div><div className="top-actions"><button className="notification" onClick={() => setNotice('You have one verified buyer offer waiting for review.')}><Bell size={20}/><i/></button><button className="help-button" onClick={resetDemo}>Reset demo</button></div></header>
      ...
      <section id="price-comparison" className="section-header"><div><p className="eyebrow">BETTER SELLING DECISIONS</p><h2>Mandi vs online store prices</h2></div><button className="text-button" onClick={() => setNotice(`Online prices updated for ${filters.crop}.`)}>Refresh comparison <ArrowRight size={16}/></button></section><PriceComparison crop={filters.crop} mandiPrice={bestMarket} onlinePrices={onlinePrices}/>
      ...
      <section id="smart-loans" className="loan-banner"><div className="loan-banner-icon"><Landmark size={24}/></div><div><p className="eyebrow">SMART FINANCE</p><h2>Need funds for your crop or business?</h2><p>Compare SBI, Axis Bank and other lender options with an eligibility-style recommendation before you apply.</p></div><button className="primary-button" onClick={() => setShowLoanForm(true)}>Apply for a smart loan <ArrowRight size={17}/></button></section>
    ...
    {showLotForm && <LotModal crop={filters.crop} onClose={() => setShowLotForm(false)} onSave={createLot}/>} {showLoanForm && <LoanModal onClose={() => setShowLoanForm(false)} onSubmitted={(application) => { setShowLoanForm(false); setLoanReceipt(application); setNotice(`Loan application ${application.id} submitted successfully.`); }}/>} {loanReceipt && <LoanReceipt application={loanReceipt} onClose={() => setLoanReceipt(null)}/>} 
  </div>;
}
