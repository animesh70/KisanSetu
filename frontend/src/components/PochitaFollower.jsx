...
export default function PochitaFollower() {
  ...
  useEffect(() => {
    const initialPosition = {
      x: Math.max(16, window.innerWidth - 40),
      y: Math.min(Math.max(16, 40), window.innerHeight - 16),
    };
    pos.current = initialPosition;
    mouse.current = { ...initialPosition };
    setIsMounted(true);
