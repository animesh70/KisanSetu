...
router.patch('/:id', requireRole('farmer', 'fpo'), (req, res) => {
  ...
  if (offer.status === 'accepted') transactions.push({ id: `txn-${Date.now()}`, lotId: offer.lotId, acceptedOfferId: offer.id, farmerId: offer.farmerId, buyerId: offer.buyerId, amount: offer.pricePerUnit * offer.quantity, status: 'confirmed', paymentStatus: 'pending' });
  res.json(offer);
});
